...
/**
  * Few Utility functions for extending Spark Datasets
  *
  * Created by anish on 22/10/17.
  */
object Utils extends Matchers {

  implicit class dataset[T](val dataset: Dataset[T]) {
  ...
  implicit class pairdatasetcms[K: Ordering : CMSHasher, V](dataset: Dataset[(K, V)]) {
  ...
  implicit class datasetcms[T: Ordering : CMSHasher](dataset: Dataset[T]) {

    /**
      * Defined on a Dataset of T, creates and returns a CMS for keys of type T
      *
      * @param eps
      * @param delta
      * @param seed
      * @return
      */
    def getCMS(eps: Double = 0.005, delta: Double = 1e-8, seed: Int = 1): CMS[T] = {
      val cmsMonoid = CMS.monoid[T](eps, delta, seed)
      dataset.rdd.map(t => cmsMonoid.create(t)).reduce(cmsMonoid.plus(_, _))
    }
  }
}